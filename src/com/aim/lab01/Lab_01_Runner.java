...
public class Lab_01_Runner extends TestFrame {
	...
	public Lab_01_Runner(Lab01TestFrameConfig config) {
	...
	public void runTests() {
		...
		List<Integer> oHeuristicIds = oRunData.stream().map( o -> o.getHeuristicId()).distinct().collect(Collectors.toList());
	...
	public RunData runExperiment(int run, int heuristicId) {
		...
		while (!sat.hasTimeExpired()) {
			...
			System.out.println(sat.getSolutionAsString(0));
	...
	public static void main(String[] args) {
		...
		Lab01TestFrameConfig config = Lab01TestFrameConfig.getInstance();
		TestFrame runner = new Lab_01_Runner(config);
		runner.runTests();
	}
}
