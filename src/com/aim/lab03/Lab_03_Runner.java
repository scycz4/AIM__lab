...
public class Lab_03_Runner extends TestFrame {
	 ...
	 * @param config
	 */
	public Lab_03_Runner(TestFrameConfig config) {
	...
	public void runTests() {
		...
		List<Integer> oHeuristicIds = oRunData.stream().map(o -> o.getHeuristicId()).distinct()
	...
	private void addBestTrace(List<RunData> oRunData, List<PlotData> oProgressPlotData, int id) {
		...
		double best = oRunData.stream().filter(f -> f.getHeuristicId() == id).mapToDouble(o -> o.getBestSolutionValue()).min().getAsDouble();
	...
	private void addWorstTrace(List<RunData> oRunData, List<PlotData> oProgressPlotData, int id) {
		...
		double best = oRunData.stream().filter(f -> f.getHeuristicId() == id).mapToDouble(o -> o.getBestSolutionValue()).max().getAsDouble();
		RunData data = oRunData.stream().filter(f -> f.getHeuristicId() == id).filter(f -> f.getBestSolutionValue() == best).findFirst().get();
	...
	public List<RunData> runExperimentsForHeuristicId(int heuristicId) {
		...
		Stream<RunData> dat = runUsingExperimentalParallelism(rangeAsStream(0, getTestConfiguration().getTotalRuns() - 1)).map( run -> {
			...
			SearchMethod searchMethod = new SimulatedAnnealing(((Lab03TestFrameConfig)getTestConfiguration())
	...
	public static void main(String [] args) {
		...
		Lab03TestFrameConfig config = Lab03TestFrameConfig.getInstance();
		TestFrame runner = new Lab_03_Runner(config);
		runner.runTests();
	}
}
